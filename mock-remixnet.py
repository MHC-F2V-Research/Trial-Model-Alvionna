...
train_dataset = FlarpDataset(vision_image_paths, force_image_paths, transform = transforms.Compose([Rescale((256, 256)), ToTensor()])) #Rescale((256,256))
...
test_dataset = FlarpDataset(vision_test_paths, force_test_paths, transform = transforms.Compose([Rescale((256,256)), ToTensor()])) #Rescale((256,256))
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(int(nsf/2), 1, (5,5), stride=1, padding=1, bias=False),
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(128 * 7 *7, ngf * 2, (5,5), stride=2, padding=1, bias=False),
            nn.Tanh(),
            #nn.Upsample((2,2), mode = 'nearest'), #mode = nearest, bilinear, bicubic and trilinear
            #nn.ConvTranspose2d(ngf * 2, ngf, (5,5), 2, 1, bias False),

            nn.Conv2d(ngf * 2, 1, (5,5), stride=2, padding=1, bias=False),
...
def train():
    ...
    for epochs in range(num_epochs):
        for i, data in enumerate(train_loader, 0):
            ...
            print("initial = " + str(img0.size()))
            print("initial = " + str(img1.size()))
            img0 = img0.view(img0.shape[0], -1)
            img1 = img1.view(img1.shape[0], -1) 
            # print(img0.size())
            # print(img1.size())
            optimizerE.zero_grad()
            img0 = torch.reshape(img0, (3, 3, 256, 256))
            img1 = torch.reshape(img1, (3, 3, 256, 256))
            # img0 = torch.unsqueeze(img0, 2)
            # img1 = torch.unsqueeze(img1, 2)
            print("unsqueeze =" + str(img0.size()))
            print("unsqueeze =" + str(img1.size()))
            output1,output2 = encoder(img0,img1)
            print('size of output1 =' + str(output1.size()))
            print('size of output2 =' + str(output2.size()))
            output1 = torch.reshape(output1, (1, 253, 253, 3))
            output2 = torch.reshape(output2, (1, 253, 253, 3))
            loss_encoder = criterion_contrastive(output1,output2,label)
            #loss_triplet  = criterion_triplet(output1, output2,label)
            loss_encoder.backward()
            #loss_triplet.backward()
            optimizerE.step()
            print('lalallaalallalalalalalalalaksjjkdshgflalfksdhfgwe8itf')
